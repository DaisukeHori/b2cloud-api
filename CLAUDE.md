...
**B2クラウドの元 JavaScript 3本をローカルに取得して参照できます**（著作権の都合でリポジトリには含まれない、`.gitignore` 済み）。
...
これで `references/` 配下に以下が配置されます:

| ファイル | サイズ | 内容 |
|---------|-------|------|
| `main-9d4c7b2348.js` | ~416KB | **B2クラウドアプリロジック**（f2a/e2a/t2m/t2m2, $.b2fetch, MPUploader, バリデーション全般）|
| `vdr-3ee93e23a5.js` | ~342KB | jQuery + plugins バンドル |
| `vdr2-3010403877.js` | ~501KB | **msgpack + zlib_asm バンドル**（ワイヤーフォーマット確認用）|

### 使い方

設計書に書かれていない細かい挙動や、関数の完全な実装が気になるときはここを直接 grep してください:
...
# b2fetch の中身
grep -boE "b2fetch\.\w+" references/main-9d4c7b2348.js | head

# MPUploader クラス
grep -b "MPUploader=function" references/main-9d4c7b2348.js
```

**⚠️ 著作物なので直接コピペして TypeScript にせず、設計書を介して独自実装してください。** 「挙動が正しいか迷ったら元JSを grep で確認 → 設計書に追記 → 自前実装」のフローを守る。

詳細は [references/README.md](./references/README.md) 参照。
