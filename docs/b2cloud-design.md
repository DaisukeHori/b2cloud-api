...
> **2026-04-17 更新:** Vercel ファイルベースルーティング → Express Router に移行済み。
> MCP は @modelcontextprotocol/sdk (StreamableHTTPServerTransport) に移行済み。
...
│   │   ├── session.ts         # B2 セッション自動作成ミドルウェア
│   │   └── error.ts           # エラーハンドリングミドルウェア
│   ├── routes/
│   │   ├── health.ts          # GET  /api/health
│   │   ├── mcp.ts             # GET/POST /api/mcp（MCP SDK transport）
│   │   ├── b2.ts              # B2 全エンドポイント統合（11ルート）
│   │   └── download.ts        # GET  /api/b2/download（署名認証、API キー不要）
│   ├── swagger.ts             # swagger-jsdoc + Swagger UI（CDN 方式）
│   ├── server.ts              # MCP SDK サーバー定義（createServer + 12ツール登録）
...
│   ├── print.ts               # 印刷/PDF取得（createAndPrint/reprintFullFlow含む）
│   ├── settings.ts            # general_settings read-modify-write（setPrinterType）
│   ├── validation.ts          # 入力バリデーション（Zod）
│   ├── signed-url.ts          # HMAC-SHA256 署名付きダウンロード URL（60秒有効）
│   ├── mcp-tools.ts           # MCP ツールハンドラ + TOOL_DESCRIPTIONS
│   ├── types.ts               # TypeScript型定義
│   ├── express.d.ts           # Express Request 型拡張（b2session プロパティ）
│   └── utils.ts               # ユーティリティ
├── tests/                     # 10 ファイル / 80 テスト
├── lp/                        # LP（GitHub Pages）
│   ├── index.html             # ランディング
│   ├── docs.html              # ドキュメント
│   └── specs.html             # 送り状仕様詳細
├── docs/
│   ├── b2cloud-design.md      # 本設計書
│   └── express-migration.md   # Express 移行設計書
├── vercel.json
├── package.json
├── tsconfig.json
├── .env.example
