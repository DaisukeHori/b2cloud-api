...
## LP を GitHub Pages で公開

`public/index.html` はそのまま GitHub Pages でも公開できます。

### 初回セットアップ

1. リポジトリの **Settings → Pages** を開く
2. **Source** を **"GitHub Actions"** に変更（"Deploy from a branch" ではない）
3. `main` に `public/` の変更を push すると `.github/workflows/pages.yml` が自動で走りデプロイ

手動でトリガーする場合は **Actions → Deploy LP to GitHub Pages → Run workflow** で実行可。

公開 URL: `https://<owner>.github.io/b2cloud-api/`
